            ...
            if(index1!=undefined)
            {
                 while(errVn[j]!=""&&errVn[j]!=undefined)
                    {   index2=FindX(Vn,errVn[j],Vnnum);
                        ...
                        if(index2==1)//有交集
                        {   
                            str3+="归约项-归约项 I"+i+"中的Follow("+errVn[t]+")与Follow("+errVn[j]+")有交集"+'\n';
                        }
                        j++;
                    }
            }
        ...
        {   var kk=xflag1;
           ...
           while(A[i][j]!=""&&A[i][j]!=undefined)
           {
            ...
            if(Xstr.charAt(index+1)!=""&&Xstr.charAt(index+1)!=undefined&&j!=kk)//移进-归约冲突
            {
                //j++;
                addL(err1,""+i+"");
                index=SLR_LR1panduan(errVn,Xstr.charAt(index+1));
                if(index!=-1)//此处判断是LR1文法
                {
                        //alert("I"+i+"项目集中")
                        str3+="移进项-归约项 I"+i+"中Follow("+errVn[index]+")与"+Xstr.charAt(index+1)+"的交集不为空"+'\n';
                                ...
                                if(i==1&&vn[j]=="#")str4+='<td>'+"acc"+'</td>';
                                    ...
                                    /////////////////////////
                                          var jcc=0,indexflag=0;
                                            ...
                                            if(indexflag==0)Xstr=LR1_match(i,vn[j]);
                                            else if(indexflag==1)Xstr=LR1_match(LALR[jcc],vn[j]);
                                ////////////////////////////////////
...
function LR1_match(a,b)//LR1 表格内容匹配
{   var cc=a;
    if(a.length>1)a=cc.charAt(0);
